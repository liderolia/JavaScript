...
const divEl = document.querySelector(".slider");
...
window.addEventListener("load", () => {
  ...
  dotArray.forEach((item) => {
    if (parseInt(item.getAttribute(`dot`)) === numberImg) {
...
dots.addEventListener("click", (e) => {
  if (e.target.tagName === "svg") {
    const number = parseInt(e.target.getAttribute(`dot`));
    selectDot(number);
    numberImg = number;
    changeClass(number);
  }
...
function selectDot(number) {
  ...
  dotArray.forEach((item) => {
    if (parseInt(item.getAttribute(`dot`)) === number) {
      item.classList.add("colorDot");
    }
  });
}
